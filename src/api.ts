import type { OinkGridParams, ProfileQueryParams } from './types/api.types';
  ...
  const response = await fetch(
    "https://nastykinkpigs.com/api/organized-chat.php?action=inbox&limit=100&filter=all&sort=date_desc",
    {
      headers: {
        ...
        priority: "u=1, i",
        "sec-ch-ua":
          '"Not A;Brand";v="99", "Chromium";v="148", "Google Chrome";v="148"',
        "sec-ch-ua-mobile": "?1",
...
export const fetchThread = async (thread_id: string) => {
    const response = await fetch(`https://nastykinkpigs.com/api/organized-chat.php?action=thread&thread_id=${thread_id}`, {
  "headers": {
    "accept": "*/*",
    "accept-language": "en-US,en;q=0.7",
    "cache-control": "no-cache",
...
export const sendMessage = async (csrf: string, recipient_id: string, text: string, thread_id: string) => {
    const bodyObj = {
        action: "send",
        _csrf: csrf,
        recipient_id,
...
export async function fetchProfileData({
  ...
  lng, 
  action = "profile" 
}: ProfileQueryParams): Promise<any> {
  const baseUrl = "https://nastykinkpigs.com/mobile/oink/index.php";
